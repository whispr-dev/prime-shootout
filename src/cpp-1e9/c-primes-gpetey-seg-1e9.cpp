...
// Odd-only base sieve up to limit (returns primes incl 2).
static std::vector<u32> base_sieve(u32 limit) {
    if (limit < 2) return {};
    const u32 size = (limit >> 1) + 1;
    std::vector<u64> bits((size + 63) / 64, ~0ULL);
    bit_clear(bits, 0); // 1 not prime
...
static u64 segmented_count(u64 n, u32 seg_odds = 1u << 20) {
    if (n < 2) return 0;
    if (n == 2) return 1;
    if (seg_odds == 0) seg_odds = 1u << 20;

    const u64 root64 = (u64)std::sqrt((long double)n);
    const u32 root = (root64 > (u64)UINT32_MAX) ? UINT32_MAX : (u32)root64;
    auto primes = base_sieve(root);
    ...
    for (u64 s = 0; s < num_segs; ++s) {
        const u64 low = 3 + 2 * s * (u64)seg_odds;
        u64 high = low + 2 * (u64)seg_odds;
        if (high > n + 1) high = n + 1;
        if (high <= low) continue;

        const u64 len_odds = (high - low) >> 1;
...
int main(int argc, char** argv) {
    ...
    std::cout << "π(" << n << ") = " << segmented_count(n, seg_odds) << "\n";
    #ifdef _OPENMP
